...
$base = 'http://www.sysinternals.com';
$subdir = '/Utilities';
$root = 'd:/sysinternals';
$dir = $root . '/www.sysinternals.com';
$dest_root = $root . '/nsis';
$scandir = $dir . $subdir;
$files = array();
	...
	foreach($exes as $exe) {
		...
		switch ($exetype) {
			case 2:
				...
				$hash[$exetype] = sprintf('!insertmacro DownloadGUI 2 "%s"	"%s"	"%s"	""	# "%s" %s' . "\n", $name, $url, $exebase, $md5, $referer);
				break;
			case 3:
				if (isset($hash[$exetype])) {
				...
				$hash[$exetype] = sprintf('!insertmacro DownloadCLI 2 "%s"	"%s"	"%s"	# "%s" %s' . "\n", $name, $url, $exebase, $md5, $referer);
				break;
			default:
				fprintf(STDERR, "exetype=%s for %s\n", $exetype, $exe);
				$hash[1] = sprintf('#!insertmacro DownloadCLI 2 "%s"	"%s"	"%s"	# "%s" %s' . "\n", $name, $url, $exebase, $md5, $referer);
				break;
		}
	}
	if (isset($hash[3])) {
